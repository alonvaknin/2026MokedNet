...
CREATE TABLE `hours_entries` (
  ...
  `requires`          ENUM('both','in','out','type') NOT NULL DEFAULT 'both',
...
מגדיר מה המנהל דורש מהנציג להשלים, ובכך מגדיר מתי השורה "הושלמה":

| ערך | משמעות | השורה עוברת ל־`filled` כאשר |
|---|---|---|
| `both` | דרושות כניסה ויציאה | שני השדות מלאים |
| `in` | דרושה שעת כניסה בלבד | `time_in` מלא |
| `out` | דרושה שעת יציאה בלבד | `time_out` מלא |
| `type` | דרוש סוג היעדרות | `entry_type` שונה מ־`regular` |

בכל מקרה: בחירת `entry_type` שאינו `regular` (חופש/מילואים/מחלה/מחיקת
כפולים/אחר) מסמנת את השורה כהושלמה ומאפסת את שדות השעות — אין מה לדווח.

השדה קיים כדי לכסות את מקרה הקצה שבו המנהל רוצה שדה אחד בלבד; בלעדיו שורה
כזו הייתה נשארת "פתוחה" לנצח.
...
- רק השדה ש־`requires` מצביע עליו פעיל.
- בחירת סוג שאינו "רגיל" מאפרת את שדות השעות.
...
- **לחיצה על תא** פותחת מודל צד: כל שורות אותו עובד+תאריך, עריכה, ו־
  "+ הוסף דרישה" עם השדות סוג / כניסה / יציאה / `requires` / הערה.
...
`canManageHours` **אינו** מקנה אוטומטית `canReportHours` — מנהל שגם מדווח
מקבל את שניהם.

**אכיפה קריטית:** בכל endpoint של נציג, השאילתה מסננת `WHERE user_id = ?`
מתוך `Auth::user()` — לעולם לא מתוך פרמטר של הבקשה. נציג לא יכול לקרוא או
לשנות שורה של נציג אחר גם אם ינחש `id`.
...
- **ולידציית זמנים:** `time_out` מוקדם מ־`time_in` באותה שורה → שגיאה
  "שעת יציאה מוקדמת משעת כניסה". (משמרת לילה מדווחת כשתי שורות.)
...
4. בחירת "מילואים" → שדות השעות מאופרים, השורה `filled`.
