...
- **הרשאות:** `canManageHours` בלבד (רשומה בשתי הרשימות ב-`UserModel` —
  המקובצת ל-UI ו-`PERM_LABELS` לשמירה; מפתח שחסר באחת מהן לא ניתן
  להענקה או לא נשמר).
- **מי מדווח נקבע פרטנית** בדגל `users.hours_reports` (צ'קבוקס "מדווח שעות"
  במסך המשתמשים), ולא לפי קבוצת הרשאות. לוח המנהל מציג רק משתמשים
  פעילים עם הדגל דלוק, ו-`HoursController::isReporter()` שומר על מסלולי
  הנציג. מנהל דיווח רשאי תמיד.
